...
loadFilePredicates(Lista):-
	open_kb('/opt/codigo/prolog/RepresentacionConocimiento/src/kb.txt',KB),
	Lista = KB.
	%write(KB),nl,
...
%addIteratorLinks(link1,[link1=>"Class2",link1=>"Class3",link1=>"Class4"],L)
addIteratorLinks(_,[],[]):-!.
addIteratorLinks(LinkName,[H|T],L):-
	=>(LinkName,_) = H,
	addIteratorLinksObject(LinkName,T,R),
	addElement([H],R,L),!.
addIteratorLinks(LinkName,[H|T],L):-
	=>(LinkName,_) \= H,
	addIteratorLinksObject(LinkName,T,L).

addIfExtendsLinksObjects(_,[],[]):-!.
addIfExtendsLinksObjects(LinkName,[H|T],L):-
	addIfExtendsLinksObjects(LinkName,T,P),
	findClassObject(H,ClassName),
	getLinksOfClass(ClassName,S),
	findLinksObject(H,Links),
	principioExpecificidad(Links,S,R),write(R),nl,!.
	%addIteratorLinksObject(LinkName,H,R,W),
	%addElement(W,P,L).
/*
*
...
%principioEspecificidadLinks([attr4=>value5,attr4=>value6,attr1=>value2],[attr1=>value5,attr3=>value4,attr2=>value2,attr4=>value7],[attr1=>value5,attr3=>value4,attr2=>value2,attr4=>value7],L).
principioEspecificidadLinks([],[],[],[]):-!.
principioEspecificidadLinks([H|T],[_|_],[],L):-addElement([H|T],[],L),!.
principioEspecificidadLinks([H|T],[],[],L):-addElement([H|T],[],L),!.
principioEspecificidadLinks([],[_|_],[W|X],L):-addElement([W|X],[],L),!.
principioEspecificidadLinks([H|T],[Y|Z],[W|X],L):-
	elimina(H,[W|X],R),
	principioEspecificidadLinks(T,[Y|Z],R,L),!.	
...
getAllLinksOfClass(ClassName,L):-
	findClassByName(ClassName,Class),
	findParentClass(Class,ParentName),
	findLinksClass(Class,Links),
	ParentName \= ClassName,!,
	getAllLinksOfClass(ParentName,R),
	addElement(Links,R,L).
getAllLinksOfClass(_,[]):-!.

%getLinksOfClass("Gato",L).
getLinksOfClass(ClassName,L):-
	findClassByName(ClassName,Class),
	findParentClass(Class,ParentName),
	findLinksClass(Class,Links),
	ParentName \= ClassName,!,
	getLinksOfClass(ParentName,L),
	getAllLinksOfClass(ParentName,D),
	principioEspecificidadLinks(Links,D,D,L).
getLinksOfClass(_,[]):-!.
